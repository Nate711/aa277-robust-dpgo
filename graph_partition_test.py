  ...
  for i in range(DISTRIBUTED_CONSENSUS_ITERS):
    ...
    for agent in graphs:
      ...
      for prior in range(noise_model.sqrt_precision_diagonal.shape[0]):
        ...
        other_agent_id = {'agent A': 0, 'agent B': 1, 'agent C': 2}[other_agent]
        if USE_ONP:
          noise_model.sqrt_precision_diagonal[prior, :] = jax.numpy.ones_like(
              noise_model.sqrt_precision_diagonal[prior, :]) * prior_precisions[
                  agent_id] * prior_precisions[other_agent_id] / jnp.mean(
                      prior_precisions)
        else:
    ...
    for agent in graphs:
      ...
      for i in range(prior_factor_stack.mu.unit_complex_xy.shape[0]):
        ...
        node_solution = graph_solutions[-1][agent_id].get_stacked_value(
            jaxfg.geometry.SE2Variable).unit_complex_xy[agent_node_id, :]
        ...
        if (USE_ONP):
          ...
          prior_factor_stack.mu.unit_complex_xy.at[i, :].set(new_factor_mu)
