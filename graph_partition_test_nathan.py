...
# FUCK ARE VARIABLES ENTERED IN ORDER OF APPEARANCE IN FACTORS?
# Need to keep track of order in which between factors are added to each agent
# Then use that to index later on into solution array
...
for i in range(OUTER_LOOP_ITERS):
    ...
    for agent in graphs:
        ...
        for i in range(prior_factor_stack.mu.unit_complex_xy.shape[0]):
            ...
            # sometimes gets the wrong node id here
            # print("Agent:", agent)
            # print(comms[agent][i])
